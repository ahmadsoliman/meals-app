...
export class MealsApiService {
  ...
  private static setMealsParams(
    ...
    if (dateRange) {
      if (dateRange.start) {
        params = params.set("startDate", dateRange.start.toLocaleDateString());
      }
      if (dateRange.end) {
        params = params.set("endDate", dateRange.end.toLocaleDateString());
      }
    }
    if (timeRange) {
      if (timeRange.start) {
        params = params.append("startTime", timeRange.start.toISOString());
      }
      if (timeRange.end) {
        params = params.append("endTime", timeRange.end.toISOString());
      }
    }
    if (skip) {
  ...
  public getMeals(
    ...
    const params = MealsApiService.setMealsParams(dateRange, timeRange, skip);
    return this.http
